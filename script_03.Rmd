...
We need to clearly define our measurements, and our goals in the data: what are we trying to find, and how are we going to measure it? 
We need to make assumptions that avoid the infinite preceptor, limiting the amount of missing data we face with the data table (enrollment on September 1st vs September 21st). For example, we made an assumption regarding what class sizes were important to us, filtering to only select data greater than 10; we realized the arbitrary nature of this, and decided that removing the filter would be advantageous toward getting a better understanding of all Harvard classes. 
We need to identify the goals of those people who are not data scientists, and how we should shape our data to reflect their goals. 


After we have a model, Temperance suggests we should:

Temperance suggests that we understand the limits of our data and our modeling capacity, acknowledging uncertainty in our calculations and the potential for incorrectly estimating a value.
We cannot make causal assumptions or assume that the same treatment effect applies consistently across all individuals.
We need to ensure that our data is valid; we should be careful to avoid various biases, including selection biases.
We need to avoid selection bias in the assignment and selection mechanisms so as to avoid confounding the data.
We need to make sure our code is correct, eliminating some level of human error in order to best promote internal validity. 
